...
public class Scp106PlayerScript : NetworkBehaviour
{
	...
	private void UpdatePointText()
	{
		...
		this.pointsText.text = ((!(PlayerPrefs.GetString("langver", "en") == "pl")) ? "SPECIAL ABILITY" : "UMIEJĘTNOŚĆ SPECJALNA");
	}
	...
	private void AnimateHighlightedText()
	{
		...
		if (PlayerPrefs.GetString("langver", "en") == "pl")
		{
			...
			{
				this.highlightedString = "STWÓRZ PRZEJŚCIE";
			}
			if (this.highlightID == 2)
			{
				this.highlightedString = "UŻYJ PRZEJŚCIA";
			}
		}
		else
		{
			if (this.highlightID == 1)
			{
				this.highlightedString = "CREATE SINKHOLE";
			}
			if (this.highlightID == 2)
			{
				this.highlightedString = "USE SINKHOLE";
			}
		}
	...
	private IEnumerator DoTeleportAnimation()
	{
		if (this.portalPrefab != null && !this.goingViaThePortal)
		{
			...
			{
				base.GetComponent<PlayerStats>().Explode();
			}
