			...
			public bool MoveNext()
			{
				...
				else
				{
					...
					for (int j = 0; j < this.<lineCount>__1; j++)
					{
						...
						Quaternion q = Quaternion.Euler(0f, 0f, UnityEngine.Random.Range(-0.25f, 0.25f) * this.$this.RotationMultiplier);
						for (int k = firstCharacterIndex; k <= lastCharacterIndex; k++)
						{
							if (this.<textInfo>__0.characterInfo[k].isVisible)
							{
								...
								this.<copyOfVertices>__0[materialReferenceIndex][vertexIndex] = vertices[vertexIndex] - b;
								this.<copyOfVertices>__0[materialReferenceIndex][vertexIndex + 1] = vertices[vertexIndex + 1] - b;
								this.<copyOfVertices>__0[materialReferenceIndex][vertexIndex + 2] = vertices[vertexIndex + 2] - b;
								this.<copyOfVertices>__0[materialReferenceIndex][vertexIndex + 3] = vertices[vertexIndex + 3] - b;
								float d = UnityEngine.Random.Range(0.995f - 0.001f * this.$this.ScaleMultiplier, 1.005f + 0.001f * this.$this.ScaleMultiplier);
								this.<matrix>__2 = Matrix4x4.TRS(Vector3.one, q, Vector3.one * d);
