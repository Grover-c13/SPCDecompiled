		...
		private sealed class <Start>c__Iterator0 : IEnumerator, IDisposable, IEnumerator<object>
		{
			...
			public bool MoveNext()
			{
				...
				case 0u:
					...
					{
						this.$this.m_textMeshPro = this.$this.gameObject.AddComponent<TextMeshProUGUI>();
						...
						this.$this.m_material02 = (Resources.Load("Fonts & Materials/LiberationSans SDF - BEVEL", typeof(Material)) as Material);
					}
					else if (this.$this.BenchmarkType == 1)
					{
						this.$this.m_textMesh = this.$this.gameObject.AddComponent<Text>();
						if (this.$this.TextMeshFont != null)
						{
							this.$this.m_textMesh.font = this.$this.TextMeshFont;
						}
						this.$this.m_textMesh.fontSize = 48;
						this.$this.m_textMesh.alignment = TextAnchor.MiddleCenter;
					}
					this.<i>__1 = 0;
					break;
				case 1u:
				...
				else
				{
					if (this.$this.BenchmarkType == 0)
					{
						this.$this.m_textMeshPro.text = "The <#0050FF>count is: </color>" + this.<i>__1 % 1000;
