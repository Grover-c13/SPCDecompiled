	...
	public class WarpTextExample : MonoBehaviour
	{
		...
		private sealed class <WarpText>c__Iterator0 : IEnumerator, IDisposable, IEnumerator<object>
		{
			...
			public bool MoveNext()
			{
				...
				while (this.$this.m_TextComponent.havePropertiesChanged || this.<old_CurveScale>__0 != this.$this.CurveScale || this.<old_curve>__0.keys[1].value != this.$this.VertexCurve.keys[1].value)
				{
					this.<old_CurveScale>__0 = this.$this.CurveScale;
					this.<old_curve>__0 = this.$this.CopyAnimationCurve(this.$this.VertexCurve);
					...
					if (this.<characterCount>__1 != 0)
					{
						...
						for (int i = 0; i < this.<characterCount>__1; i++)
						{
							if (this.<textInfo>__1.characterInfo[i].isVisible)
							{
								...
								float num2 = (vector.x - this.<boundsMinX>__1) / (this.<boundsMaxX>__1 - this.<boundsMinX>__1);
								float num3 = num2 + 0.0001f;
								float y = this.$this.VertexCurve.Evaluate(num2) * this.$this.CurveScale;
								float y2 = this.$this.VertexCurve.Evaluate(num3) * this.$this.CurveScale;
								Vector3 lhs = new Vector3(1f, 0f, 0f);
								Vector3 rhs = new Vector3(num3 * (this.<boundsMaxX>__1 - this.<boundsMinX>__1) + this.<boundsMinX>__1, y2) - new Vector3(vector.x, y);
